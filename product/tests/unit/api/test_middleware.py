...
class TestJWTMiddleware:
    ...
    def test_jwk_to_pem_invalid_key(self, jwt_middleware):
        """Test JWK to PEM with invalid key data."""
        jwk_key = {
            "n": "AA==",  # Too short modulus (results in n=0, which is < 3)
            "e": "AQAB"
        }
        
        with patch('api.middleware.logging') as mock_logging:
            with pytest.raises(ValueError, match="n must be >= 3"):
                jwt_middleware.jwk_to_pem(jwk_key)
    ...
    @pytest.mark.asyncio
    async def test_verify_jwt_token_invalid_key_id(self, jwt_middleware, sample_jwks):
        """Test JWT token verification with invalid key ID."""
        with patch.object(jwt_middleware, 'get_jwks', return_value=sample_jwks), \
             patch('api.middleware.settings') as mock_settings, \
             patch('api.middleware.logging') as mock_logging:
            
            mock_settings.product_key_id = "wrong-key-id"
            
            result = await jwt_middleware.verify_jwt_token("test-token")
            
            assert result is None
            mock_logging.error.assert_called_once()
            assert "Invalid key ID in JWKS" in mock_logging.error.call_args[0][0]

    @pytest.mark.asyncio
    async def test_verify_jwt_token_invalid_key_type(self, jwt_middleware):
        """Test JWT token verification with invalid key type."""
        invalid_jwks = {
            "keys": [
                {
                    "kid": "test-key-id",
        ...
        with patch.object(jwt_middleware, 'get_jwks', return_value=invalid_jwks), \
             patch('api.middleware.settings') as mock_settings, \
             patch('api.middleware.logging') as mock_logging:
            ...
            assert result is None
            mock_logging.error.assert_called_once()
            assert "Invalid key type or algorithm" in mock_logging.error.call_args[0][0]

    @pytest.mark.asyncio
    async def test_verify_jwt_token_expired(self, jwt_middleware, sample_jwks):
        ...
        with patch.object(jwt_middleware, 'get_jwks', return_value=sample_jwks), \
            ...
            assert result is None
            mock_logging.error.assert_called_once_with("JWT token has expired")
    ...
    @pytest.mark.asyncio
    async def test_dispatch_bearer_without_token(self, jwt_middleware, middleware_mock_request):
        """Test middleware dispatch with Bearer but no token."""
        middleware_mock_request.headers = {"Authorization": "Bearer "}  # Space but no token
        mock_call_next = AsyncMock()
        mock_response = Mock()
        mock_call_next.return_value = mock_response
        
        with patch.object(jwt_middleware, 'verify_jwt_token', return_value=None) as mock_verify:
            result = await jwt_middleware.dispatch(middleware_mock_request, mock_call_next)
            
            assert result == mock_response
            assert middleware_mock_request.state.jwt_payload is None
            assert middleware_mock_request.state.is_authenticated is False
            # Should call verify_jwt_token with empty string
...
class TestHTTPExceptionHandler:
    ...
    @pytest.mark.asyncio
    async def test_http_exception_handler_basic(self, mock_request):
        """Test basic HTTP exception handler functionality."""
    ...
    @pytest.mark.asyncio
    async def test_http_exception_handler_different_status_codes(self, mock_request):
    ...
    async def test_http_exception_handler_with_headers(self, mock_request):
        """Test HTTP exception handler with custom headers."""
        exc = HTTPException(
            status_code=401,
    ...
    @pytest.mark.asyncio
    async def test_http_exception_handler_different_paths(self):
        """Test HTTP exception handler with different request paths."""
        test_paths = [
            "/",
            "/api/products",
            "/api/products/1",
            "/api/origamis/1/votes",
            "/health",
        ]
    ...
    @pytest.mark.asyncio
    async def test_http_exception_handler_long_detail_message(self, mock_request):
        """Test HTTP exception handler with long detail message."""
        long_detail = "This is a very long error message that contains multiple sentences. " * 10
    ...
    @pytest.mark.asyncio
    async def test_http_exception_handler_special_characters(self, mock_request):
        """Test HTTP exception handler with special characters in detail."""
        special_detail = "Error with special chars: àáâãäåæçèéêë & <script>alert('xss')</script>"
        exc = HTTPException(status_code=400, detail=special_detail)
